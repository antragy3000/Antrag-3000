# ============================================================
# Antrag 3000 – FOERDERER-App: eine neue Version in EINEM Befehl veroeffentlichen.
#
# Vorbild: release/veroeffentlichen.ps1 der Nutzer-App. Nutzt DENSELBEN
# Signatur-Schluessel (der Update-Pubkey in foerderer-app/src-tauri/
# tauri.conf.json ist identisch mit dem der Nutzer-App), liefert die Pakete
# aber aus dem NAS-Unterordner  updates/foerderer/  aus, damit sich Foerderer-
# und Nutzer-Updates nie vermischen.
#
# Unterschiede zur Nutzer-App: KEIN Benutzerhandbuch-Schritt (die Foerderer-App
# buendelt kein Handbuch), und der Ziel-Unterordner wird vor dem Upload einmalig
# angelegt (beim allerersten Foerderer-Release existiert er noch nicht).
#
# Macht nacheinander:
...
#   4. den Ziel-Ordner updates/foerderer/ auf der NAS anlegen (falls neu),
...
#
# Aufruf (im foerderer-app-Ordner):
...
param(
  ...
  # Foerderer-Updates liegen im UNTERORDNER "foerderer" (getrennt von der Nutzer-App).
  [string]$UpdatesPfad = "/home/antrag/Antrag-3000/server/updates/foerderer",
  [string]$KeyDatei    = "$HOME\.tauri\antrag3000.key",
  # Endpoint nur zur Schluss-Pruefung (muss zu tauri.conf.json passen).
  [string]$PruefUrl    = "https://sync.antrag3000.de/updates/foerderer/latest.json"
)

$ErrorActionPreference = "Stop"
# Ins foerderer-app-Stammverzeichnis wechseln (eine Ebene ueber diesem Skript).
...
Write-Host "Foerderer-Version laut tauri.conf.json: $version" -ForegroundColor Green
...
# --- 2. Signiert bauen ---------------------------------------------------
# createUpdaterArtifacts steht in der Foerderer-tauri.conf.json auf true, daher
...
# Beim allerersten Foerderer-Release existiert updates/foerderer/ noch nicht;
# scp wuerde dann scheitern. mkdir -p ist harmlos, wenn der Ordner schon da ist.
...
Write-Host "`nFertig: Foerderer-Version $version ist veroeffentlicht. Foerderer-Geraete mit" -ForegroundColor Green
