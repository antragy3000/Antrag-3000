// Erzeugt foerderer-app/release/latest.json aus den signierten Build-
// Artefakten der FOERDERER-App. Gleiches Prinzip wie release/latest-json-
// bauen.mjs der Nutzer-App (das Vorbild), nur mit dem Foerderer-Unterordner
// der NAS, damit sich Foerderer-Updates nicht mit denen der Nutzer-App
// vermischen.
//
...
//
// Aufruf (die Release-Notiz wird im Update-Dialog angezeigt):
...
// WICHTIG: Foerderer-Updates liegen im UNTERORDNER "foerderer", damit sie sich
// nicht mit den Updates der Nutzer-App (Wurzel "/updates") vermischen.
const ENDPOINT_BASIS = "https://sync.antrag3000.de/updates/foerderer";
...
if (!setup || !sig) {
  ...
  console.error("Und steht createUpdaterArtifacts in tauri.conf.json auf true?");
  process.exit(1);
}
...
const ziel = path.join(wurzel, "release", "latest.json");
fs.writeFileSync(ziel, JSON.stringify(manifest, null, 2));

console.log(`latest.json geschrieben (Foerderer-Version ${version}).`);
console.log("");
console.log("Jetzt auf die NAS in den Ordner  updates/foerderer/  hochladen:");
console.log(`  1. ${setup}`);
console.log(`  2. foerderer-app/release/latest.json`);
